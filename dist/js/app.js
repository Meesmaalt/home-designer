...
let dragHandle = null; // { wallId, end: 1|2 }
const SNAP_EPS = 0.2;
...
/** Nurkade snap – ühenda lähedased otspunktid */
function snapPoint(x, z, excludeWallId = null) {
  ...
  walls.forEach(w => {
    ...
    (w.openings || []).forEach(op => {
      ...
      if (op.type === 'door') {
        ...
        const g = new THREE.Group();
        const pts = [
          new THREE.Vector3(ox - Math.cos(angle) * half, y, oz - Math.sin(angle) * half),
          new THREE.Vector3(ox + Math.cos(angle) * half, y, oz + Math.sin(angle) * half),
        ];
        ...
        for (let i = 0; i <= 12; i++) {
          const a = angle - Math.PI / 2 + (swing * i) / 12;
          const r = op.width || 0.9;
          arcPts.push(new THREE.Vector3(
            ox + Math.cos(a) * r * 0.15 + nx * 0.02,
            y,
            oz + Math.sin(a) * r * 0.15 + nz * 0.02
          ));
        }
        // lihtsam: veerandring
        const arc = [];
        ...
        // aken: topeltjoon seina suhtes risti
        const g = new THREE.Group();
        const off = 0.08;
        for (const s of [-1, 1]) {
...
function detectRoomsFromWalls(keepNames = true) {
  ...
  function normalizeCycle(cyc) {
    if (cyc.length < 3) return null;
    // rotate so min key first, direction canonical
    let minI = 0;
    for (let i = 1; i < cyc.length; i++) if (cyc[i] < cyc[minI]) minI = i;
    const rot = cyc.slice(minI).concat(cyc.slice(0, minI));
    const rot2 = rot.slice().reverse();
    const r2 = rot2.slice(-1).concat(rot2.slice(0, -1)); // keep start
    let minI2 = 0;
    for (let i = 1; i < r2.length; i++) if (r2[i] < r2[minI2]) minI2 = i;
    const rotB = r2.slice(minI2).concat(r2.slice(0, minI2));
    const s1 = rot.join('>'), s2 = rotB.join('>');
    return s1 < s2 ? s1 : s2;
  }
  ...
  // filter: only simple cycles with area > 0.8 m², prefer smaller faces (not outer if huge)
  const polys = [];
  cycles.forEach(cyc => {
  ...
  // remove polys that contain others? keep all reasonable; sort by area
  polys.sort((a, b) => a.area - b.area);
  // drop near-duplicates (same center)
  const filtered = [];
  polys.forEach(p => {
  ...
  rooms = filtered.map((p, i) => {
    ...
    const areaStr = p.area.toFixed(2).replace('.', ',');
    if (!name.includes('m²')) name = `${name} ${areaStr} m²`;
    else name = name.replace(/\d+[.,]\d+\s*m²/, areaStr + ' m²');
    return { id: uid(), name, x: p.x, z: p.z, area: p.area };
  });
  rebuildRooms();
  refreshQuote();
  return rooms;
...
function buildPlanSvg() {
  ...
  if (!walls.length) { minX = 0; maxX = L; minZ = 0; maxZ = W; }
  const pad = 1.0;
  minX -= pad; maxX += pad; minZ -= pad; maxZ += pad;
  const bw = maxX - minX, bh = maxZ - minZ;
  const Wsvg = 720, Hsvg = Math.max(320, Math.round(720 * (bh / bw)));
  const sx = (x) => ((x - minX) / bw) * (Wsvg - 40) + 20;
  const sz = (z) => ((z - minZ) / bh) * (Hsvg - 40) + 20;
  let s = `<svg xmlns="http://www.w3.org/2000/svg" width="${Wsvg}" height="${Hsvg}" viewBox="0 0 ${Wsvg} ${Hsvg}">`;
  s += `<rect width="100%" height="100%" fill="#fafafa"/>`;
  s += `<text x="20" y="18" font-size="14" fill="#333" font-family="system-ui">${escapeHtml(document.getElementById('project-name')?.value || 'Plaan')}</text>`;
  walls.forEach(w => {
    s += `<line x1="${sx(w.x1)}" y1="${sz(w.z1)}" x2="${sx(w.x2)}" y2="${sz(w.z2)}" stroke="#333" stroke-width="4" stroke-linecap="square"/>`;
    const len = wallLength(w);
    const mx = (w.x1 + w.x2) / 2, mz = (w.z1 + w.z2) / 2;
    s += `<text x="${sx(mx)}" y="${sz(mz) - 6}" font-size="11" fill="#c06020" text-anchor="middle" font-family="system-ui">${len.toFixed(2)} m</text>`;
    const angle = Math.atan2(w.z2 - w.z1, w.x2 - w.x1);
    (w.openings || []).forEach(op => {
      const ox = w.x1 + Math.cos(angle) * op.along;
      const oz = w.z1 + Math.sin(angle) * op.along;
      const col = op.type === 'door' ? '#1a6a2a' : '#2a6a9a';
      s += `<circle cx="${sx(ox)}" cy="${sz(oz)}" r="5" fill="${col}"/>`;
      s += `<text x="${sx(ox) + 8}" y="${sz(oz) + 4}" font-size="10" fill="${col}" font-family="system-ui">${op.type === 'door' ? 'U' : 'A'}</text>`;
    });
  });
  rooms.forEach(r => {
...
function exportPdfPlan() {
  refreshQuote();
  const name = document.getElementById('project-name')?.value || 'Projekt';
  const svg = buildPlanSvg();
  const roomRows = rooms.map(r =>
    `<tr><td>${escapeHtml(r.name)}</td><td style="text-align:right">${(r.area != null ? r.area : '—')}</td></tr>`
  ).join('') || '<tr><td colspan="2">Ruumid puuduvad – vajuta „Tuvasta ruumid“</td></tr>';
  let wallM2 = 0, doors = 0, windows = 0;
  walls.forEach(w => {
    wallM2 += wallLength(w) * (w.h || H);
    (w.openings || []).forEach(o => { if (o.type === 'door') doors++; else windows++; });
  });
  const html = `<!DOCTYPE html><html lang="et"><head><meta charset="utf-8">
<title>Plaan – ${escapeHtml(name)}</title>
<style>
  @page { margin: 12mm; }
  body { font-family: system-ui, sans-serif; color: #222; margin: 0; padding: 1rem; }
  h1 { font-size: 1.35rem; margin: 0 0 0.25rem; }
  .meta { color: #666; font-size: 0.9rem; margin-bottom: 1rem; }
  .plan { border: 1px solid #ddd; border-radius: 8px; overflow: hidden; margin: 1rem 0; }
  ...
  <p class="noprint"><button onclick="window.print()">Prindi / salvesta PDF</button></p>
  <h1>${escapeHtml(name)}</h1>
  <p class="meta">KoduDisain · ${new Date().toLocaleDateString('et-EE')} · seinu ${walls.length} · uksi ${doors} · aknaid ${windows}</p>
  <div class="plan">${svg}</div>
  <div class="grid">
    <div>
      <h2>Ruumid</h2>
      <table><thead><tr><th>Nimi</th><th>m²</th></tr></thead><tbody>${roomRows}</tbody></table>
    </div>
    <div>
      <h2>Pakkumine (ori.)</h2>
      <div>${document.getElementById('quote-box')?.innerHTML || ''}</div>
      <p class="meta">Seinad ~${wallM2.toFixed(1)} m² · orienteeriv ilma KM-ta</p>
    </div>
  </div>
  <p class="meta">Legendi: U = uks, A = aken · mõõdud meetrites</p>
</body></html>`;
  const win = window.open('', '_blank');
  if (!win) { setStatus('Popup blokeeritud'); return; }
  win.document.write(html);
  win.document.close();
  setStatus('PDF vaade avatud – Ctrl+P');
}
...
function placeTemplate() {
  ...
  rooms = [
    { id: uid(), name: 'Puhkeruum 10,9 m²', x: FRONT_D / 2, z: W / 2, area: 10.9 },
    { id: uid(), name: 'Pesu 2,11 m²', x: FRONT_D + BACK_D / 2, z: PESU_W / 2, area: 2.11 },
    { id: uid(), name: 'Leili 5,84 m²', x: FRONT_D + BACK_D / 2, z: PESU_W + LEILI_W / 2, area: 5.84 },
  ];
  rebuildRooms();
  rebuildPlanSymbols();
...
function refreshQuote() {
  ...
  el.innerHTML = `
    <div><span>Seinad · ${wallM2.toFixed(1)} m²</span><strong>${matCost.toFixed(0)} €</strong></div>
    <div><span>Uksed ${doors} · aknad ${windows}</span><strong>${openCost.toFixed(0)} €</strong></div>
    <div><span>Sisustus</span><strong>${furnCost.toFixed(0)} €</strong></div>
    <div><span>Töö · ${PRICES.laborPct}%</span><strong>${labor.toFixed(0)} €</strong></div>
    <div class="q-total"><span>Hinnang kokku</span><strong>~${total.toFixed(0)} €</strong></div>`;
}
...
function renderCatalog(query = '') {
  ...
  catEl.querySelectorAll('[data-add]').forEach(b => {
    b.onclick = () => {
      ...
      if (o) { select(o); pushHist(); refreshList(); refreshQuote(); setStatus('Lisatud: ' + (item?.name || b.dataset.add)); }
    };
  });
}
renderCatalog();
...
function scrollCatalogTo(category) {
  if (catalogSearch?.value) { catalogSearch.value = ''; renderCatalog(); }
  catEl.querySelector(`[data-category="${category}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
canvas.addEventListener('pointermove', e => {
  if (dragHandle) {
    ...
    if (!w) return;
    const sn = snapPoint(p.x, p.z, w.id);
    if (dragHandle.end === 1) { w.x1 = sn.x; w.z1 = sn.z; }
    else { w.x2 = sn.x; w.z2 = sn.z; }
    const len = wallLength(w);
    (w.openings || []).forEach(op => { op.along = Math.min(Math.max(op.along, 0.1), Math.max(len - 0.1, 0.1)); });
    rebuildWallMesh(w);
    rebuildDims();
    showWallHandles(w.id);
    return;
...
document.getElementById('btn-detect-rooms')?.addEventListener('click', () => {
  ...
  setStatus(r.length ? `Tuvastatud ${r.length} ruumi` : 'Suletud ruume ei leitud – kontrolli seinte ühendusi');
});
...
function setViewMode(mode) {
  ...
  if (sceneLabel) sceneLabel.textContent = mode === '3d' ? '3D vaade' : mode === 'blueprint' ? 'Plaanivaade' : '2D vaade';
  ...
  controls.update();
  setStatus(mode === '3d' ? '3D vaade' : mode === 'blueprint' ? 'Plaanivaade – mõõdud ja joonis' : '2D – sobib seinte joonistamiseks');
}
